...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CSR)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CSR")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeCsrAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCsrAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCsrAmount = strAnonymizeCsrAmount.remove(strAnonymizeCsrAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCsrAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCsrAmount * COIN) nMaxToAnonymize = nAnonymizeCsrAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCsrAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCsrAmount));
        strAnonymizeCsrAmount = strAnonymizeCsrAmount.remove(strAnonymizeCsrAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCsrAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
